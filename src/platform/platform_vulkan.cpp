...
int PlatformVulkan::console_width() {
    int cols = 0;
#ifdef _WIN32
...
void PlatformVulkan::remove_console() {
#ifdef _WIN32
    FreeConsole();
#endif
}
...
void PlatformVulkan::platform_init() {
...
bool PlatformVulkan::is_down(SDL_Scancode key) {
    return keybuf[key];
}

void PlatformVulkan::platform_shutdown() {
...
void PlatformVulkan::complete_frame() {
...
void PlatformVulkan::begin_frame() {
...
void PlatformVulkan::strcpy(char* dst, const char* src, size_t limit) {
#ifdef _WIN32
...
void PlatformVulkan::loop(App& app) {
...
Vec2 PlatformVulkan::scale(Vec2 pt) {
    return pt * window_draw() / window_size();
}

Vec2 PlatformVulkan::window_size() {
    int w, h;
    SDL_GetWindowSize(window, &w, &h);
    return Vec2((float)w, (float)h);
}

Vec2 PlatformVulkan::window_draw() {
    int w, h;
    SDL_GL_GetDrawableSize(window, &w, &h);
    return Vec2((float)w, (float)h);
}

void PlatformVulkan::grab_mouse() {
    SDL_SetWindowGrab(window, SDL_TRUE);
}

void PlatformVulkan::ungrab_mouse() {
    SDL_SetWindowGrab(window, SDL_FALSE);
}

Vec2 PlatformVulkan::get_mouse() {
    int x, y;
    SDL_GetMouseState(&x, &y);
    return Vec2(x, y);
}

void PlatformVulkan::capture_mouse() {
    SDL_CaptureMouse(SDL_TRUE);
    SDL_SetRelativeMouseMode(SDL_TRUE);
}

void PlatformVulkan::release_mouse() {
    SDL_CaptureMouse(SDL_FALSE);
    SDL_SetRelativeMouseMode(SDL_FALSE);
}

void PlatformVulkan::set_mouse(Vec2 pos) {
